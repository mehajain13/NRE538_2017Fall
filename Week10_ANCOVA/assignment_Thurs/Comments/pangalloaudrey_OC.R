...
##### Oscar: This is not correct. 
#####        The correst interpretation is as follow. 
#####        Root length has a significantly positive effect (23.24) on fruit weight. 
#####        Ungrazed treatment has almost significant higher fruit weight than grazed treatment. 
#####        [Note that I wrote the above interpreation because the interaction term is not significant.]
#####        The effects (i.e. regression coefficient) of root length (grazing treatment) is not being influenced by grazing treatment (root length). 
...
modsol= lm(Temp~Wind*Solar.R, data=airquality)
summary(modsol)
...
##### Oscar: because you did not load the interplot library...

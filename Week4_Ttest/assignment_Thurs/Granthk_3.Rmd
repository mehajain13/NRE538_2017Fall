...
Exercise 2 Correct Method: 
```{r}
dif.g=c()
for (i in 1:10000){
  dif.g[i]=mean(sample(guns.WAS[,"murder"],nrow(guns.WAS), replace=TRUE)-sample(guns.MIN[,"murder"],nrow(guns.MIN), replace = TRUE))
}
...
```
This code here allows us to run a two sample comparison between Murder rates in Washington and Minnesota. Then we calculate the 95% confidence interval. We then use the last code to calculate the probability that we will observe a value greater than the zero distribution. In this case, our probability was 2, so we fail to reject the null hypothesis.  
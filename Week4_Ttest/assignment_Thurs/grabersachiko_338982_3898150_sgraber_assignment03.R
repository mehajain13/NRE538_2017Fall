...
#murder rate in WA is significantly different than MN
#here, we want a 2-tailed two value unpaired t-test
...
#We reject the null hypothesis at the 1% significance level. The murder rate in WA is significantly different than MN

hist(guns.WAS[,"murder"],probability=TRUE,xlim=c(0,8),col="light blue",main="Histogram of murder rates",xlab="murder rate",ylim=c(0,1))
lines(density(guns.WAS[,"murder"]),col="blue")
hist(guns.MIN[,"murder"],probability=TRUE,xlim=c(0,8),col="light green",add=TRUE,main="",xab="")
lines(density(guns.MIN[,"murder"]),col="dark green")
legend("topleft",legend=c("Washington","Minnesota","99% CI, WA"),text.col=c("blue","dark green","red"),cex=0.8)
mean.WAS=mean(guns.WAS[,"murder"])
sd.WAS=sd(guns.WAS[,"murder"])
abline(v=c(qnorm(0.005,mean=mean.WAS,sd=sd.WAS),qnorm(0.995,mean=mean.WAS,sd=sd.WAS)),col="red",lty="dashed")
abline(v=mean(guns.MIN[,"murder"]),col="dark green",lwd=3)
